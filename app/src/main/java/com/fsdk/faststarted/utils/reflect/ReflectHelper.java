package com.fsdk.faststarted.utils.reflect;
...
public final class ReflectHelper {
    ...
    public Object newInstance(ReflectArgument... args) {
        ...
        if (constructor != null) {
            try {
                ...
                XLog.e(TAG, "newInstance error=%s", e);
            }
        }
        return null;
    }
    ...
    private static Field getClassField(Class<?> target, String fieldName) {
        ...
        if (value == NOT_FOUND) {
            XLog.d(TAG, "getClassField NOT_FOUND %s", key);
            return null;
        }
    ...
    private static Constructor getClassConstructor(Class<?> target, Class[] paramTypes) {
        ...
        Object value = CACHE.get(key);
        if (value == NOT_FOUND) {
            XLog.d(TAG, "getClassConstructor NOT_FOUND %s", key);
            return null;
        }
        ...
        if (constructor == null) {
            try {
                ...
                XLog.e(TAG, "getClassConstructor NOT_FOUND %s error=%s", key, e);
            }
    ...
    private static Method getClassMethod(Class<?> target, String methodName, Class[] paramTypes) {
        ...
        Object value = CACHE.get(key);
        if (value == NOT_FOUND) {
            XLog.d(TAG, "getClassMethod NOT_FOUND %s", key);
            return null;
        }
    ...
    static Class getClassForName(String className) {
        ...
        if (value == NOT_FOUND) {
            XLog.d(TAG, "getClassForName NOT_FOUND %s", className);
            return null;
        }
        ...
        if (clazz == null) {
            try {
                ...
                XLog.e(TAG, "getClassForName NOT_FOUND %s error=%s", className, e);
            }
